...
transform.params <- function(params){
  ...
  return(c(beta.0.t, alpha.1.t, beta.1.t, m.1.t, pi.t))
}
...
inv.transform.params <- function(params){
  ...
  pi <- 1/(1+exp(-params[5]))
  return(c(beta.0, alpha.1, beta.1, m.1, pi))
}
...
#transform.params <- function(params){
#  beta.0.t <- - log(params[1]); alpha.1.t <- - log(((1-params[3])/params[2])-1)
#  beta.1.t <- - log((1/params[3])-1); m.1.t <- - log(params[4])
#  pi.t <- - log((1/params[5]) - 1)
#  return(c(beta.0.t, alpha.1.t, beta.1.t, m.1.t, pi.t))
#}
...
#inv.transform.params <- function(params){
#  beta.0 <- exp(-params[1]); beta.1 <- 1/(1+exp(-params[3])); 
#  alpha.1 <- (1-beta.1)/(1+exp(-params[2])); m.1 <- exp(-params[4])
#  pi <- 1/(1+exp(-params[5]))
#  return(c(beta.0, alpha.1, beta.1, m.1, pi))
#}
...
col.nlm.obj <- fit_country("colombia", neg.loglik.nb.ingarch,
                           ...
                           params=c(0.01, 0.05, 0.8, 3.4, 0.02), N.iter=20, sigma=0.1)
...
colnames(est.matrix) <- c("beta.0", "alpha.1", "beta.1", "m", "pi")
...
est.matrix[, 5] <- round(est.matrix[, 5], 4)
est.matrix
##################################

#           beta.0 alpha.1 beta.1     m     pi
# Colombia 8.7e-02  0.0463 0.9449   5.4 0.0394
# Uganda   5.5e-10  0.0406 0.9592   1.6 0.0177
# Congo    6.5e-01  0.1101 0.8437   0.0 0.0072
# Ethiopia 1.8e+01  0.0850 0.5795 606.7 0.0011
...
col.est
#[1] 0.08714199 0.04627278 0.94490500 5.42018300 0.03938773
uga.est
#[1] 5.530259e-10 4.057004e-02 9.592238e-01 1.595643e+00 1.773562e-02
con.est
#[1] 6.451809e-01 1.101413e-01 8.436932e-01 1.601460e-06 7.232661e-03
eth.est
#[1] 1.787846e+01 8.500927e-02 5.795386e-01 6.066816e+02 1.079615e-03
...
#            RPS      QS
# Colombia  6.13 -0.1987
# Uganda    3.60 -0.6656
# Congo    12.03 -0.5049
# Ethiopia 49.05 -0.4853
